...
String generatePeriodProgress(int startDate, int period, String durationType) {
  switch(durationType) {
    case 'Hari':
      double dayTh = DateTime.fromMillisecondsSinceEpoch(startDate).difference(DateTime.now()).inDays / 1;
      return "$dayTh Hari / $period Hari";
    case 'Pekan':
      double weekTh = DateTime.fromMillisecondsSinceEpoch(startDate).difference(DateTime.now()).inDays / 7;
      return "$weekTh Pekan / $period Pekan";
    case 'Bulan':
      double monthTh = DateTime.fromMillisecondsSinceEpoch(startDate).difference(DateTime.now()).inDays / 30;
      return "$monthTh Bulan / $period Bulan";
    default:
      double yearTh = DateTime.fromMillisecondsSinceEpoch(startDate).difference(DateTime.now()).inDays / 365;
